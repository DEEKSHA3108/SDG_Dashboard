...
def load_data_and_forecasts():
    ...
    for country in ["India", "Germany"]:
        ...
        series = country_df['gdp_growth'].dropna()
        order = find_best_arima_order(series)
        forecasts[country] = hybrid_forecast_plotly(
            df, country, forecast_horizon=5, show_legend=(country=="India"), arima_order=order
        )
    return df, forecasts

def find_best_arima_order(series, p_values=[0,1,2], d_values=[0,1], q_values=[0,1,2]):
    """
    Finds the best ARIMA order (p,d,q) for a given time series using AIC.
    """
    import warnings
...
# Forecasting Function — Direct Multi-step
def hybrid_forecast_plotly(df, country_name, forecast_horizon=15, direct_years=5):
    target = 'gdp_growth'
    country_df = df[df["Country"] == country_name].copy()
    country_df = country_df.drop(columns=["Country", "gdp_per_capita_growth"], errors="ignore")
    country_df = create_lag_features(country_df).dropna().reset_index(drop=True)
    ...
    # Prepare XGB future predictions
    last_data = country_df.copy()
    ...
    for i, year in enumerate(future_years):
        ...
        # First 'direct_years' years: use ARIMA's prediction for lags
        if i < direct_years:
            new_row_pred = xgb.predict(pd.DataFrame([new_row])[feature_cols])[0]
            temp_data = pd.concat(
                [temp_data, pd.DataFrame([{target: arima_future.iloc[i]}])],
                ignore_index=True
            )
        else:
            # Recursive mode: use XGB's own prediction for lags
            new_row_pred = xgb.predict(pd.DataFrame([new_row])[feature_cols])[0]
            temp_data = pd.concat(
                [temp_data, pd.DataFrame([{target: new_row_pred}])],
                ignore_index=True
            )

        future_preds_xgb.append(new_row_pred)

    # Combine hybrid predictions
    hybrid_future = 0.6 * np.array(future_preds_xgb) + 0.4 * arima_future
    ...
    # Build full forecast dataframe
    forecast_years = np.concatenate([country_df['Year'], future_years])
    forecast_vals = np.concatenate([
        0.6 * full_preds_xgb + 0.4 * arima_full_preds[:len(country_df)],
        hybrid_future
    ])

    # Trend line
    trend_df_hist = pd.DataFrame({'Year': forecast_years, target: forecast_vals})
    trend_df_hist = trend_df_hist[trend_df_hist['Year'] >= 1995]
